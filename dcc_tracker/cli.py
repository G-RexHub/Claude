...
@click.pass_context
def cli(ctx, state_file, config_file):
...
def _load(ctx) -> tuple[dict, dict, dict]:
    """Helper: load state + orchestrations, return (state, all_orchs, config_path)."""
    ...
    return state, all_orchs, state_path
...
def log(ctx, orchestration_name, count):
    ...
    state, all_orchs, state_path = _load(ctx)
    try:
...
def report(ctx, output_dir):
    ...
    sr_totals = tracker.calculate_sr_totals(all_orchs, state["estimates"])
    thresholds = tracker.calculate_thresholds(sr_totals)
    rows = tracker.build_report_rows(sr_totals, thresholds)

    if not rows:
        click.echo("No service request data to report. Set estimates first with `set-estimate`.")
        return
    ...
    col_w = max(len(r["Service Request Type"]) for r in rows)
    header = (
        f"{'Service Request Type':<{col_w}}  "
    ...
    for row in rows:
        click.echo(
            f"{row['Service Request Type']:<{col_w}}  "
...
@click.option(
    ...
    multiple=True,
    type=(str, int),
    metavar="SR_TYPE COUNT",
    required=True,
    help="Service request type and count per run. Repeatable.",
)
@click.pass_context
def add_orchestration(ctx, orchestration_name, sr_pairs):
    ...
    Example:
      dcc-tracker add-orchestration My_Orch \\
        --sr ECS05_Read_Import_Registers 2 \\
        --sr CS01_Read_Device_Log 1
    """
    state, all_orchs, state_path = _load(ctx)
